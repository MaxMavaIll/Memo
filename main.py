...
async def process_network(
    ...
    try: 
        ...
        data_memo_address_time = await cosmos.Get_Block_Memo(
            ...
            address_user=cache_users[id_network], 
            settings_json=data)
            ...
            for address in data_memo_address_time[height]:
                ...
                if answer_delegate.get('message') == "Transaction was successfully registered!": 
                    if data_memo_address_time[height][address]['typeId'] == 1:
                        log.info(f"{id_log} | {name_network.get('name')} \
                                 ->  User: {address} Delegate: {data_memo_address_time[height][address]['amount']}")
                        user_delegates[id_network][memo_id][address] += float(
                            data_memo_address_time[height][address]['amount'])
                    
                    elif data_memo_address_time[height][address]['typeId'] == 2 \
                            and user_delegates[id_network][memo_id][address] >= float(
                            data_memo_address_time[height][address]['amount']):
                        log.info(f"{id_log} | {name_network.get('name')} \
                                 ->  User: {address} UnDelegate: {data_memo_address_time[height][address]['amount']}")
                        user_delegates[id_network][memo_id][address] -= float(
                            data_memo_address_time[height][address]['amount'])
