...
def to_tmpstmp_mc(
        date_str: str = '2023-09-30T09:26:05Z') -> int:
...
async def check_rpc(
            ...
            async with session.get(url, ssl=config_toml['ssl']) as response:
                if response.status == 200:
                    ...
                    if (data["result"]["sync_info"]["catching_up"] == True 
                            or data['result']['sync_info']['latest_block_height'] == 0 
                            or now_height == last_height):
    ...
    if settings['network'][name_network]['rpc'] == "" \
            or settings['network'][name_network]['rest'] == "":
